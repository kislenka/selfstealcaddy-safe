...
6. Настраивает UFW (открывает `80`, `2222`, `443`, `8443`) и показывает статус прослушивания портов
...
| Порт | Сервис | Доступ |
|------|--------|--------|
| `2222` | Доп. TCP порт | Публичный — открывается скриптом |
| `443` | Xray Reality | Публичный — VPN-подключения |
| `80` | Caddy | Публичный — обновление сертификата + заглушка |
| `8443` | Caddy HTTPS | Открывается в firewall, но сам Caddy слушает только `127.0.0.1` |
